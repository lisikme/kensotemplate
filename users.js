...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Индивидуальный запрос без кэша
    async function fetchSingleDiscordUser(discordId, retryCount = 0) {
        if (!discordId || !discordId.match(/^\d{17,19}$/)) return null;
        ...
        try {
            // НЕТ cache buster - просто обычный URL
            const url = `${config.discordApiBase}${discordId}`;
    ...
    // Batch запрос - ТОЛЬКО для обновления кэша
    async function fetchMultipleDiscordUsers(discordIds) {
        const uniqueIds = [...new Set(discordIds.filter(id => id && typeof id === 'string' && id.match(/^\d{17,19}$/)))];
        if (uniqueIds.length === 0) return [];
        ...
        // Разбиваем на маленькие пачки по 5 ID
        const chunkSize = 5;
    ...
    async function loadDiscordAvatar(discordId, elementId, username) {
        ...
        const promise = (async () => {
            try {
                ...
                // Обновление аватара ТОЛЬКО если есть данные в кэше
                const userData = getDiscordUserDataFromCache(discordId);
                ...
                if (userData && userData.avatar && avatarElement) {
                    ...
                    if (!isSameAvatar && avatarElement.src !== './images/none.png') {
                        let avatarUrl = null;
                        const discordCdnGif = `https://cdn.discordapp.com/avatars/${discordId}/${avatarHash}.gif`;
                        const discordCdnPng = `https://cdn.discordapp.com/avatars/${discordId}/${avatarHash}.png`;
    ...
    function loadCacheFromLocalStorage(discordIds) {
        ...
        console.log(`Загружено ${discordDataCache.size} записей из кэша`);
    }
    
    // ОБНОВЛЕНИЕ КЭША - вызывается раз в час
    async function updateDiscordCache() {
        ...
        try {
            // Собираем все Discord ID из пользователей
            const allDiscordIds = [];
            draftUsers.forEach(user => {
                if (user.sid && user.sid.match(/^\d{17,19}$/)) {
                    allDiscordIds.push(user.sid);
            ...
            console.log(`Обновлено ${updatedCount} записей в кэше`);
            setLastUpdateTime();
            
            // Обновляем отображение с новыми данными из кэша
            refreshDisplayWithCache();
    ...
    // Обновление отображения из кэша
    ...
    // Загрузка черновых данных (без Discord API)
    async function loadDraftData() {
        try {
            console.log('Загрузка черновых данных...');
            ...
            // Загружаем кэш из localStorage ДО отображения
            const allDiscordIds = [];
            draftUsers.forEach(user => {
                if (user.sid && user.sid.match(/^\d{17,19}$/)) {
                    allDiscordIds.push(user.sid);
            ...
            // Проверяем нужно ли обновить кэш (раз в час)
    ...
    function displayUsers(users, adminDiscordIds) {
        ...
        Promise.allSettled([...avatarPromisesList, ...usernamePromisesList]).then(() => console.log('Все данные из кэша загружены'));
    }
    
    async function init() {
        // Загружаем черновые данные (без Discord API)
        await loadDraftData();
    }
