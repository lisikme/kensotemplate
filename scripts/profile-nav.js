...
(function() {
    ...
    function getUserFromCacheSync(discordId) {
        ...
        try {
            const cached = localStorage.getItem('users_list_cache_v6');
            if (cached) {
                const cache = JSON.parse(cached);
                if (cache.users && Array.isArray(cache.users)) {
                    const user = cache.users.find(u => u.discordId === discordId);
    ...
    // Главная функция обновления профиля - МГНОВЕННО из кеша
    async function updateNavProfile(forceRefresh = false) {
        const btnLogout = document.querySelector('.btn_logout');
        const contextMenu = document.querySelector('.context-menu');
        ...
        // МГНОВЕННОЕ отображение из кеша (синхронно)
        const cachedUser = getUserFromCacheSync(String(discordUser.id));
        ...
        if (cachedUser) {
            ...
            // Если force refresh и нет кеша - загружаем из API
            profileSection.innerHTML = `
                <div class="profile-nav-loading">
                    <div class="profile-nav-spinner"></div>
                    <div>Загрузка профиля...</div>
            ...
            // Нет кеша - показываем заглушку и грузим
            profileSection.innerHTML = `
                <div class="profile-nav-loading">
                    <div class="profile-nav-spinner"></div>
                    <div>Загрузка профиля...</div>
    ...
    function preloadProfileFromCache() {
        ...
        if (cachedUser) {
            ...
            // Предзагружаем контекстное меню
            const contextMenu = document.querySelector('.context-menu');
            if (contextMenu) {
                let profileSection = contextMenu.querySelector('.context-menu-hwid');
                if (!profileSection) {
